...
var Ninja = function(options) {
  ...
  this.Block = function(options) {
    ...
    /**
     * Override the native toString function
     * @return {string} the Node Id of the Block
     */
    ...
    this.IsListening = function() {
      return isListening;
      // return (listenerXHR !== undefined && (listenerXHR.readyState === 3 || listenerXHR.readyState === 2 || listenerXHR.readyState === 1));
    };
    ...
    this.ParseCommands = function(error, response) {
      if (listenerXHR && listenerXHR.readyState === 4) {
        ...
        if (isListening) {
          // Re-listen
          if (ninja.Options.debug) {console.log("Relistening");}
          lastXHRIndex = 0;
    ...
    this.Listen = function(interval) {
      ...
      if (!listenerXHR) {
        ...
        try {
          ...
          // listenerXHR.addEventListener('load', this.ListenErrorHandler.bind(this), false);
          listenerXHR.addEventListener('loadend', this.ParseCommands.bind(this), false);
     ...
     */
    this.Stop = function() {
      if (ninja.Options.debug) {console.log("Stopping");}
      isListening = false;

      setTimeout(function() {
        try { listenerXHR.abort(); } catch (e) {}
        lastXHRIndex = 0;
        listenerXHR = undefined;
      }.bind(this), 10)
    ...
    this.BroadcastCommand = function(command) {
      ...
      if (command.DEVICE) {
        ...
        for (var i=0; i<devices.length; i++) {
          ...
          device.Options.value = commandDevice.DA;

          try {
            device.Actuate(commandDevice.DA, this, device);
          } catch (e) {
            if (ninja.Options.debug) {
              console.log("Could not actuate device", device.GUID(), "with value", DA);
            }
          }
        }
      }
    ...
    /**
     * Unregister a Device from this block
     * @param {Device} device Device to unregister
     */
    this.UnregisterDevice = function(device) {
      var removeIndex = this.Devices.indexOf(device);
    ...
    this.GetDeviceByPortVendorDeviceID = function(port, vendor, deviceId) {
      ...
      for (var i=0; i<this.Devices.length; i++) {
        ...
        if (parseInt(deviceInstance.Options.vendor, 10) === parseInt(vendor, 10) &&
          deviceInstance.Options.port.toString() === port.toString() &&
          parseInt(deviceInstance.Options.deviceId, 10) === parseInt(deviceId, 10)) {
  ...
  this.Driver = function(options) {
    ...
    /**
     * Block reference for this driver
     * @type {[type]}
     */
    this.Block = null;
    ...
    /**
     * Sets a driver configuration
     * @param {Object} config Configuration object to send to the driver
     * @param {Function} callback Optional callback
     */
    ...
    this.Driver = Ninja.Utilities.CreateXHR(function(error, response) {

    });


    /**
     * Send an RPC command to the client driver
  ...
  this.Device = function(options) {
    ...
    this.GUID = function() {
      if (this.Options.hasOwnProperty('guid')) {
        ...
        return this.Options.node + '_' + this.Options.port + '_' + this.Options.vendor + '_' + this.Options.deviceId;
      }
      else
    ...
    this.DeleteSubdevice = function(subObject, callback) {
      if (subObject instanceof ninja.Subdevice) {
        ...
        var xhr = Ninja.Utilities.CreateXHR((function(err, response) {
    ...
    if (this.Options.hasOwnProperty('guid')) {
      // Parse out the blockId
      var guidSplit = Ninja.Utilities.SplitGUID(this.Options.guid);
      this.Options.block = new ninja.Block({ nodeId: guidSplit.nodeId });
      this.AutoLoadData();
    }
