...
var Ninja = function(options) {
	...
	this.Block = function(options) {
		...
		this.Activate = function(callback) {
			var postData = { nodeid: this.Options.node_id };
			var postUrl = ninja.Options.server + '/rest/v' + ninja.Options.version + '/block/' + this.Options.node_id + '/activate';
		...
		this.Listen = function(interval) {
			...
			var commandsUrl = ninja.Options.server + '/rest/v' + ninja.Options.version + '/block/' + this.Options.node_id + '/commands';
		...
		this.GetDevicesByDeviceId = function(deviceId) {
			...
			for (var i=0; i<this.Devices.length; i++) {
				...
				if (device.Options.device_id === deviceId) {
					devices.push(device);
				}
			}
		...
		this.GetDeviceByPortVendorDeviceID = function(port, vendor, deviceId) {
			...
			for (var i=0; i<this.Devices.length; i++) {
				...
				if (deviceInstance.Options.vendor === vendor &&
					...
					deviceInstance.Options.device_id == deviceId) {
	...
	this.Device = function(options) {
		...
		this.GUID = function() {
			if (this.Options.block)
				return this.Options.block.Options.node_id + '_' + this.Options.port + '_' + this.Options.vendor + '_' + this.Options.device_id;
			else
