...
var Ninja = function(options) {
	...
	var UserAccount = function(options) {
		...
		this.GetDevices = function(callback) {
			...
			var xhr = Ninja.Utilities.CreateXHR(function(response) {
				if (response.result) {
					...
					console.log(GetDevices_Handler(response.data, callback));
		...
		var GetDevices_Handler = function(devices, callback) {
			var devicesArray = [];
			var blocksArray = [];
	...
	this.Block = function(options) {
		...
		this.Activate = function(callback) {
			var postData = { nodeid: this.Options.nodeId };
			var postUrl = ninja.Options.server + '/rest/v' + ninja.Options.version + '/block/' + this.Options.nodeId + '/activate';
		...
		this.Listen = function(interval) {
			...
			var commandsUrl = ninja.Options.server + '/rest/v' + ninja.Options.version + '/block/' + this.Options.nodeId + '/commands';
	...
	this.Device = function(options) {
		...
		this.Emit = function(data, callback) {
			...
			var url = ninja.Options.server + '/rest/v' + ninja.Options.version + '/block/' + this.Options.block.Options.nodeId + '/data' + ninja.Authentication.getAuthSlug();
			data = {
				GUID: this.GUID(),
				G: this.Options.port,
